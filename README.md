...
Current corpus: **326 documents across 16 districts, 81,667 words extracted.** By type: minutes (273, mostly Norwich's back to 2001), other (26), CCRs (6), policy (5), rates (4), bylaws (3), ordinances (3), permits (2), annual reports (2), plus budget and plan.
...
**220 of 326 documents are scanned images with no text layer — 67%.** Extraction falls back from PyMuPDF to pdfplumber and, when both yield under 200 characters, flags `needs_ocr` rather than silently recording an empty document. The site says "scanned, text not extracted" on those. OCR would need Tesseract, which is not installed here; until then those 220 are links only. Norwich alone accounts for 204 of them — its older minutes, back to 2001, are scans. Whole districts are affected beyond that: every South Alburgh document and every Wilmington CCR before 2025 is a scan.
